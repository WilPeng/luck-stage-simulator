...
const HOUSE_ROOMS = [
  ...
  { id: 'have_not_room', name: '贫民屋',      nameEn: 'Have-Not Room', icon: '🥶', type: 'lodging',  capacity: null, accessRule: 'have_not_only' },
...
const HOUSE_PASSAGES = [
  ...
  { id: 'pass_living_hoh',         from: 'living_room', to: 'hoh_room',      type: 'normal' },
...
/** Seed 函数：确保数据库中有房间/通道/门数据 */
async function seedHouseData({ BBHouseRoom, BBHousePassage, BBHouseDoor }) {
  const gameId = 'bigbrother'

  const existingRooms = await BBHouseRoom.countDocuments({ gameId })
  if (existingRooms === 0) {
    for (const r of HOUSE_ROOMS) {
      const model = new BBHouseRoom({ ...r, gameId })
      await model.save()
    }
    console.log(`[HouseMap] Seeded ${HOUSE_ROOMS.length} rooms`)
  }

  const existingPassages = await BBHousePassage.countDocuments({ gameId })
  if (existingPassages === 0) {
    for (const p of HOUSE_PASSAGES) {
      const model = new BBHousePassage({ ...p, gameId })
      await model.save()
    }
    console.log(`[HouseMap] Seeded ${HOUSE_PASSAGES.length} passages`)
  }

  const existingDoors = await BBHouseDoor.countDocuments({ gameId })
  if (existingDoors === 0) {
    for (const d of HOUSE_DOORS) {
      const model = new BBHouseDoor({ ...d, gameId })
      await model.save()
    }
    console.log(`[HouseMap] Seeded ${HOUSE_DOORS.length} doors`)
  }
}
...
async function ensureBBHouseData() {
  ...
  for (const p of houseguests) {
    const existing = await BBPlayerLocation.findOne({ playerId: p.id, gameId: 'bigbrother' })
    if (!existing) {
